...
import { createOpenAIEvidenceResolver, resolveEvidenceFirstResearchFlag, researchCandidateEvidenceFirst, type EvidenceFirstTelemetry } from './evidence-first';
...
export function generateResearchEditorial(event: ExtractedRaceEvent, metadata: Record<string, unknown> = {}, resolutions: Record<string, FieldResolution> = {}) {
  const usable = (field: string, value: unknown) => resolutions[field]?.status === 'conflicted' || resolutions[field]?.status === 'unresolved' ? null : value;
  const name = usable('name', event.name) as string | null;
  const city = usable('city', event.city) as string | null;
...
export function resolveResearchMode(override: boolean | undefined, runtimeEnabled: boolean): 'evidence_first'|'legacy_web_search' {
  return (override ?? runtimeEnabled) ? 'evidence_first' : 'legacy_web_search';
}

export async function researchAndEnrichCandidate(candidateId: string, input: ResearchInput, provider: RaceResearchProvider | undefined = undefined, options: { maxQueries?: number; maxSources?: number; client?: ReturnType<typeof adminDb>; onPersistenceStart?: () => Promise<void>; dryRunExecutionId?: string; executionStartedAt?: string; evidenceFirstEnabled?: boolean } = {}) {
  ...
  const runtimeEvidenceFirst = options.evidenceFirstEnabled === undefined ? (await resolveEvidenceFirstResearchFlag({ client })).enabled : false;
  const configuredResearchMode = options.evidenceFirstEnabled === undefined ? null : resolveResearchMode(options.evidenceFirstEnabled, runtimeEvidenceFirst);
  const effectiveResearchMode = resolveResearchMode(options.evidenceFirstEnabled, runtimeEvidenceFirst);
  const evidenceFirst = effectiveResearchMode === 'evidence_first';
  ...
  const criticalConflictFields = new Set(Object.entries(decision.fieldResolutions).filter(([field, resolution]) => criticalFields.has(field) && (resolution.status === 'conflicted' || resolution.status === 'unresolved')).map(([field]) => field));
  const criticalConflictValues = [...result.conflicts.filter((conflict) => criticalConflictFields.has(conflict.field)).flatMap((conflict) => conflict.values.map((value) => value.value)), ...Object.entries(decision.fieldResolutions).filter(([field]) => criticalConflictFields.has(field)).flatMap(([, resolution]) => resolution.conflictingSources.map((source) => source.title).filter(Boolean))];
  ...
  const inputTokens = evidenceFirst ? (evidenceTelemetry.inputTokens ?? null) : (result.inputTokens ?? null);
  const outputTokens = evidenceFirst ? (evidenceTelemetry.outputTokens ?? null) : (result.outputTokens ?? null);
  ...
  const payload = { candidate_id: candidateId, base_event: input.event, enriched_event: merged, research_sources: { accepted: scoredSources.filter((source) => source.accepted), rejected: [...rejectedSources, ...scoredSources.filter((source) => !source.accepted)] }, field_evidence: result.fieldEvidence, conflicts: result.conflicts, missing_fields: result.missingFields, research_confidence: safeEditorial.researchConfidence, content_quality_score: decision.contentQualityScore, short_description: safeEditorial.shortDescription, long_description: safeEditorial.longDescription, model: result.model || null, input_tokens: inputTokens, output_tokens: outputTokens, total_tokens: inputTokens === null || outputTokens === null ? null : inputTokens + outputTokens, field_resolutions: decision.fieldResolutions, replacements: decision.replacements, unresolved_conflicts: decision.unresolvedConflicts, factual_confidence: decision.factualConfidence, auto_publish_eligible: autoPublishEligible && !editorialContainsConflict, rejection_reasons: [...rejectionReasons, ...(editorialContainsConflict ? ['critical_conflict_in_editorial'] : [])], description_audit: { supportedClaims: audit.supportedClaims, unsupportedClaims: audit.unsupportedClaims, referencedUrls: audit.referencedUrls }, research_metadata: { ...metrics, ...executionMetadata, executionFinishedAt: new Date().toISOString(), configuredResearchMode, effectiveResearchMode, researchMode: effectiveResearchMode, webSearches: result.webSearches ?? 0, responseShape: result.responseShape ?? {}, ...evidenceTelemetry, inputTokens, outputTokens, totalTokens: inputTokens === null || outputTokens === null ? null : inputTokens + outputTokens, sourceMatchScores: scoredSources.map(({ url, sourceMatchScore, matchClassification, accepted, rejectionReason, domain, technicalOrigin, sourceWeight, editionMatch, excludedFromResolution, exclusionReason }) => ({ url, domain, sourceMatchScore, matchClassification, accepted, rejectionReason, technicalOrigin, sourceWeight, editionMatch, excludedFromResolution, exclusionReason })) } };
