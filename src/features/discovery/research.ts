...
import { isEvidenceFirstResearchEnabled } from './evidence-first';
...
export async function researchAndEnrichCandidate(candidateId: string, input: ResearchInput, provider: RaceResearchProvider | undefined = undefined, options: { maxQueries?: number; maxSources?: number; client?: ReturnType<typeof adminDb>; onPersistenceStart?: () => Promise<void> } = {}) {
  ...
  // The opt-in path is intentionally isolated. Until a caller supplies the
  // evidence resolver/source adapter, it falls back safely to the legacy provider.
  // This preserves the existing behavior when the flag is off and avoids
  // silently changing production research semantics.
