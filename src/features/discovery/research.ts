...
export async function researchAndEnrichCandidate(candidateId: string, input: ResearchInput, provider: RaceResearchProvider | undefined = undefined, options: { maxQueries?: number; maxSources?: number; client?: ReturnType<typeof adminDb>; onPersistenceStart?: () => Promise<void>; dryRunExecutionId?: string; executionStartedAt?: string } = {}) {
  ...
  const evidenceFirst = (await resolveEvidenceFirstResearchFlag({ client })).enabled;
