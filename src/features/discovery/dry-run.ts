...
export async function runDiscoveryDryRun(options: DiscoveryDryRunOptions = {}) {
  ...
  async function worker() {
    while (cursor < claimed.length) {
      ...
      if (!extraction.success || !extraction.extractedEvent) { items.push({ ...emptyItem(candidate, itemStarted), extractionStatus: extraction.errorCode || 'failed', durationMs: Date.now() - itemStarted }); continue; }
      ...
      if (options.enableResearch && needsResearch && researchUsed < researchLimit) {
        ...
        try {
          ...
          researchResult = await researchAndEnrichCandidate(candidate.id, input, options.researchProvider, { client: options.client as never });
