...
function emptyItem(candidate: DiscoveryCandidate, started: number, dryRunExecutionId: string): DiscoveryDryRunItem { return { candidateId: candidate.id, sourceId: candidate.source_id, url: candidate.url, dryRunExecutionId, evaluatedAt: new Date().toISOString(), persistenceStatus: 'not_persisted', persistedUpdatedAt: null, researchExecuted: false, deterministicFields: null, enrichedFields: null, missingFields: [], conflicts: [], researchConfidence: null, contentQualityScore: 0, shortDescription: '', longDescription: '', sourcesCount: 0, sourceTypes: [], durationMs: Date.now() - started, estimatedAutoPublishEligibility: false, rejectionReasons: ['research_failed'], researchTelemetry: null }; }

function persistedTelemetry(value: Record<string, unknown> | null, mode: 'evidence_first'|'legacy_web_search'|null): DiscoveryResearchTelemetry | null {
  if (!value) return null;
  const numberValue = (key: string) => typeof value[key] === 'number' && Number.isFinite(value[key]) ? Number(value[key]) : 0;
  const listValue = (key: string) => Array.isArray(value[key]) ? value[key].filter((item): item is string => typeof item === 'string') : [];
  return { researchMode: value.researchMode === 'evidence_first' || value.researchMode === 'legacy_web_search' ? value.researchMode : mode, evidenceCacheHits: numberValue('evidenceCacheHits'), evidenceCacheMisses: numberValue('evidenceCacheMisses'), evidenceDocsFetched: numberValue('evidenceDocsFetched'), evidenceDocsUsed: numberValue('evidenceDocsUsed'), evidenceChars: numberValue('evidenceChars'), deterministicFieldsResolved: numberValue('deterministicFieldsResolved'), llmFieldsRequested: listValue('llmFieldsRequested'), evidenceResolverCalled: Boolean(value.evidenceResolverCalled), evidenceResolverInputTokens: numberValue('evidenceResolverInputTokens'), evidenceResolverOutputTokens: numberValue('evidenceResolverOutputTokens'), fallbackWebSearchUsed: Boolean(value.fallbackWebSearchUsed), fallbackReasons: listValue('fallbackReasons'), fallbackInputTokens: numberValue('fallbackInputTokens'), fallbackOutputTokens: numberValue('fallbackOutputTokens'), inputTokens: numberValue('inputTokens'), outputTokens: numberValue('outputTokens'), totalTokens: numberValue('totalTokens') };
}
    ...
    while (cursor < claimed.length) {
      ...
      if (options.enableResearch && needsResearch && researchUsed < researchLimit) {
        ...
        try {
          ...
          researchResult = await researchAndEnrichCandidate(candidate.id, input, options.researchProvider, { client: options.client as never, dryRunExecutionId, executionStartedAt, evidenceFirstEnabled: options.evidenceFirstEnabled });
