...
export async function persistBatchChunk(client: Client, batch: Record<string, unknown>, items: Array<Record<string, unknown>>, positions: number[]) {
  ...
  const next = Number(batch.next_index) + items.length; const total = Number(batch.total_count); const succeeded = items.filter(i=>i.persistenceStatus==='persisted').length; const failed = items.length - succeeded; const status = next >= total ? (failed > 0 ? 'partially_completed' : 'completed') : 'running';
  const { error } = await client.from('discovery_dry_run_batches').update({ next_index: next, processed_count: next, succeeded_count: Number(batch.succeeded_count || 0) + succeeded, failed_count: Number(batch.failed_count || 0) + failed, status, finished_at: next >= total ? new Date().toISOString() : null, updated_at: new Date().toISOString() }).eq('batch_execution_id', batch.batch_execution_id); if (error) throw error;
}
