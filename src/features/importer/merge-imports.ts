...
export type ConflictField = 'startDate' | 'city' | 'state' | 'venue' | 'registrationUrl' | 'distances';
export type ImportConflict = { field: ConflictField; primary: string; secondary: string };
...
export function mergeEventImports(primary: ImportedEventDraft, secondary: ImportedEventDraft): MergedImport {
  ...
  const draft = { ...primary };
  const choose = (field: 'startDate' | 'city' | 'state' | 'venue' | 'registrationUrl') => {
  ...
  choose('startDate'); choose('city'); choose('state'); choose('venue'); choose('registrationUrl');
