...
export function extractEventMetadata(html: string, sourceUrl: string): ImportedEventDraft { const graph=jsonLd(html).find(x => {const t=x['@type']; return t === 'Event' || t === 'SportsEvent' || (Array.isArray(t) && t.some(v => v === 'Event' || v === 'SportsEvent')); }) || {}; const embedded=embeddedRecords(html); const embeddedEvent=embedded.find(x=>isRecord(x.location)||x.city||x.cidade||x.registrationUrl||x.registration_url)||{}; const loc=isRecord(graph.location)?graph.location:(isRecord(embeddedEvent.location)?embeddedEvent.location:{}); const addr=isRecord(loc.address)?loc.address:{}; const offers=isRecord(graph.offers)?graph.offers:{}; const organizer=isRecord(graph.organizer)?graph.organizer:{}; const name=clean(String(graph.name||embeddedEvent.name||embeddedEvent.title||tag(html,'meta','property','og:title')||tag(html,'meta','name','twitter:title')||titleTag(html))); const sourceDescription=sanitizeEventText(String(graph.description||embeddedEvent.description||tag(html,'meta','property','og:description')||tag(html,'meta','name','description'))); const start=dateValue(graph.startDate||embeddedEvent.startDate||embeddedEvent.date); const imageCandidates=[Array.isArray(graph.image)?graph.image[0]:graph.image, embeddedEvent.image, tag(html,'meta','property','og:image'), tag(html,'meta','name','twitter:image')]; const image=imageCandidates.map(usableImage).find(Boolean)||''; const allText=textOf(html); const portal=isPortalDasCorridas(sourceUrl); const portalPlace=portal ? portalLocation(allText) : {city:'',state:''}; const ds=distances(`${name} ${sourceDescription} ${allText}`); const embeddedCity=embeddedEvent.city||embeddedEvent.cidade||embeddedEvent.addressLocality||(isRecord(loc)?loc.city||loc.cidade:''); const embeddedState=embeddedEvent.state||embeddedEvent.uf||embeddedEvent.addressRegion||(isRecord(loc)?loc.state||loc.uf:''); const city=clean(String(addr.addressLocality||embeddedCity||portalPlace.city||'')); const state=normalizeBrazilianState(addr.addressRegion||embeddedState||portalPlace.state); const venue=clean(String(loc.name||embeddedEvent.venue||embeddedEvent.local||'')); const registrationCandidates=[offers.url, embeddedEvent.registrationUrl, embeddedEvent.registration_url, registrationLink(html)].map(value=>clean(typeof value==='string'?value:'')).filter(value=>classifyRegistrationUrl(value)==='specific'); const registrationUrl=registrationCandidates[0]||''; const generated=generateEventEditorialContent({name,startDate:start,startTime:start?start.slice(11,16):(portal ? portalTime(allText) : ''),city,state,venue,organizerName:clean(String(organizer.name||embeddedEvent.organizer||'')),distances:ds,priceFrom:offers.price?Number(offers.price):(portal ? portalPrice(allText) : null),registrationUrl}); const draft: ImportedEventDraft={name,slug:slugify(name),shortDescription:generated.shortDescription,description:generated.description||sourceDescription,startDate:start,startTime:start?start.slice(11,16):(portal ? portalTime(allText) : ''),city,state,venue,address:clean(String(addr.streetAddress||embeddedEvent.address||embeddedEvent.endereco||'')), organizerName:clean(String(organizer.name||embeddedEvent.organizer||'')),category:/trail|montanha/i.test(allText)?'trail':/night|noturna/i.test(allText)?'night':/kids|infantil/i.test(allText)?'kids':'rua',priceFrom:offers.price?Number(offers.price):(portal ? portalPrice(allText) : null),registrationUrl,regulationUrl:'',officialUrl:sourceUrl,coverImageUrl:image,distances:ds,sourceUrl,fieldsFound:[]}; for(const [key,value] of Object.entries(draft)) if(value&&(!Array.isArray(value)||value.length)) draft.fieldsFound.push(key); return draft; }
