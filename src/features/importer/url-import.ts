...
export async function fetchEventPage(rawUrl: string): Promise<ImportedEventDraft> {
  ...
  try {
    const request = (url: string) => fetch(url, { signal: controller.signal, redirect: 'manual', headers: { Accept: 'text/html,application/xhtml+xml', 'User-Agent': 'MapRunImporter/1.0' } });
    let url = originalUrl;
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (redirectTarget.origin !== new URL(url).origin) throw new Error('redirect_cross_origin_blocked');
      const redirectedUrl = await assertSafeImportUrl(redirectTarget.toString());
      url = redirectedUrl;
      response = await request(url);
      if (response.status >= 300 && response.status < 400) throw new Error('redirect_chain_blocked');
    }
    if (!response.ok) throw new Error(`Página indisponível (HTTP ${response.status}).`);
    const type = response.headers.get('content-type') || '';
    if (!type.includes('text/html') && !type.includes('application/xhtml+xml')) throw new Error('A URL não retornou uma página HTML.');
    const length = Number(response.headers.get('content-length') || 0);
    if (length > 2_000_000) throw new Error('A página excede o limite de importação.');
    const html = await response.text();
    if (html.length > 2_000_000) throw new Error('A página excede o limite de importação.');
    const result = extractEventExtraction(html, url);
    return { ...result.draft, finalUrl: url, extraction: { event: result.event, fieldSources: result.fieldSources, extractionQuality: result.extractionQuality, shouldUseAiFallback: result.shouldUseAiFallback, relevantPageText: result.relevantPageText } };
  } catch (e) {
    if (e instanceof Error && e.name === 'AbortError') throw new Error('Tempo limite excedido ao acessar a página.');
    throw e;
