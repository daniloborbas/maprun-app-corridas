-- Add manual registration availability status and preserve the complete admin RPC.\nalter table public.events add column if not exists registration_status text not null default 'open';\nupdate public.events set registration_status = 'open' where registration_status is null;\nalter table public.events drop constraint if exists events_registration_status_check;\nalter table public.events add constraint events_registration_status_check check (registration_status in ('open', 'sold_out', 'closed'));\n\n-- Add persisted deterministic feed artwork while preserving official detail artwork.
alter table public.events add column if not exists description_source text not null default 'unknown';
alter table public.events add column if not exists feed_image_url text;
...
begin
  ...
  insert into public.events(
    id, slug, name, short_description, description, description_source, feed_image_url, registration_status, start_date, end_date,
    ...
    coalesce(nullif(payload->>'description_source',''),'unknown'), payload->>'feed_image_url', coalesce(nullif(payload->>'registration_status',''),'open'), (payload->>'start_date')::timestamptz,
    ...
    description_source=excluded.description_source, feed_image_url=excluded.feed_image_url, registration_status=excluded.registration_status, slug=excluded.slug, start_date=excluded.start_date, end_date=excluded.end_date,
