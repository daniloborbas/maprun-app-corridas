-- Preserve all editable event facts when an existing event is saved.
...
begin
  ...
  insert into public.events(
    ...
    payload->>'fallback_image_key', payload->>'short_tagline', payload->>'status',
  ...
  on conflict (id) do update set
    ...
    fallback_image_key = excluded.fallback_image_key,
    published_at = case
  ...
  if jsonb_typeof(payload->'event_sources') = 'array' then
    for item in select value from jsonb_array_elements(payload->'event_sources') loop
      if not exists (
        ...
        where event_id = eid
          and lower(regexp_replace(regexp_replace(source_url, '[?#].*$', ''), '/$', '')) =
              lower(regexp_replace(regexp_replace(coalesce(item->>'source_url',''), '[?#].*$', ''), '/$', ''))
      ) then
        insert into public.event_sources(
          event_id, source_name, source_url, import_method, last_verified_at
        ) values (
          eid,
          coalesce(nullif(item->>'source_name',''),'Cadastro manual'),
          coalesce(item->>'source_url',''),
          coalesce(nullif(item->>'source_method',''),'manual'),
          now()
        );
      end if;
    end loop;
    ...
    insert into public.event_sources(
      ...
      eid,
      coalesce(nullif(payload->>'source_name',''),'Cadastro manual'),
      coalesce(payload->>'source_url',''),
      coalesce(nullif(payload->>'source_method',''),'manual'),
      now()
    );
  end if;
